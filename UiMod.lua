...
local library = {design = getgenv().design == "kali" and "kali" or "uwuware", tabs = {}, draggable = true, flags = {}, title = "tickware.", open = false, popup = nil, instances = {}, connections = {}, options = {}, notifications = {}, tabSize = 0, theme = {}, foldername = "tickware_cnfgs", fileext = ".txt"}
...
library.createSlider = function(option, parent)
    ...
    function option:SetValue(value, nocallback)
		...
		game:GetService("TweenService"):Create(option.fill, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new((value - self.min) / (self.max - self.min), 0, 1, 0)}):Play()
        else
		...
		game:GetService("TweenService"):Create(option.fill, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(value / (self.max - self.min), 0, 1, 0)}):Play()
		game:GetService("TweenService"):Create(option.fill, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(value / (self.max - self.min), 0, 1, 0)}):Play()
        end
